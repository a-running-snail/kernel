...
#define CDFINGER_SET_VERSION_INFO        _IOW(CDFINGER_IOCTL_MAGIC_NO, 18, uint64_t)
...
#ifndef DTS_PROBE
	cdfinger->spi->dev.of_node = of_find_compatible_node(NULL,NULL,"mediatek,teksun-finger");
#endif
...
/*	
	cdfinger->vdd = regulator_get(NULL, "vgp1");
	if(IS_ERR(cdfinger->vdd))
	{
		printk("cannot find regulator vgp1\n");
		cdfinger->vdd = NULL;	
	}
	regulator_set_voltage(cdfinger->vdd, 2800000, 2800000);
	ret = regulator_enable(cdfinger->vdd);
*/
/*
	cdfinger->vdd = regulator_get(&cdfinger->spi->dev, "vgp1finger");

	ret = regulator_set_voltage(cdfinger->vdd, 2800000, 2800000); 
	if (ret)
	{
			CDFINGER_ERR("%s: Could not set to 2800mv.\n", __func__);
    	}
	regulator_enable(cdfinger->vdd);
* */
...
static long cdfinger_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case CDFINGER_GETID:
			...
			break;
		case CDFINGER_SET_VERSION_INFO:
			#if defined(CONFIG_PRIZE_HARDWARE_INFO)
			{
				char version_info[32];
				if (__copy_from_user(version_info, (void __user*)arg, 32)) { 
					return -EFAULT;
				}
				sprintf(current_fingerprint_info.id,"0x98\n  lib:%s",version_info);
			}
			#endif
			ret = 0;
			break;
				...
				struct device_attribute *attr, char *buf)
{
	...
	int freq = 0;
	freq = fps_spi_speed;
	return sprintf(buf, "spi frequence[%d]\n", freq);
	#endif
}
...
static int cdfinger_probe(struct spi_device *spi)
{
	...
	if (cdfinger->sensor_type == 0x98)
		...
		sprintf(current_fingerprint_info.chip,"FPS980");
	else if (cdfinger->sensor_type == 0x56)
		sprintf(current_fingerprint_info.chip,"FPS956");
...
static int cdfinger_spi_init(void)
{
	int ret;
#ifndef DTS_PROBE 
	ret=spi_register_board_info(spi_board_cdfinger, ARRAY_SIZE(spi_board_cdfinger));
#endif
    CDFINGER_DBG("cdfinger_spi_init\r\n");
	ret=spi_register_driver(&cdfinger_driver);
	if(ret<0)
	{
	    CDFINGER_DBG("cdfinger_spi_init  fail\r\n");	
	}
	return 1;
}
