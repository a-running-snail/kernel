...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if(((info->chr_type == NONSTANDARD_CHARGER) || (info->chr_type == STANDARD_CHARGER)))
	{
		#if 1//defined(CONFIG_PROJECT_KOOBEE_K5706Q)
...
static void swchg_turn_on_charging(struct charger_manager *info)
{
	...
	if (swchgalg->state == CHR_ERROR) {
		...
		/*prize add charger hw init hjian 20171206*/
        charger_dev_charger_init(info->chg1_dev,true);
#endif
