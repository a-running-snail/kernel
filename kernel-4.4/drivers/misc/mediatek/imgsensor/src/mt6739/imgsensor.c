...
/*modify by trf318, imgsensor set driver begin*/
UINT32 kdGetSensorInitFuncList(struct IMGSENSOR_INIT_FUNC_LIST **ppSensorList)
{
	if (NULL == ppSensorList) {
		PK_PR_ERR("[kdGetSensorInitFuncList]ERROR: NULL ppSensorList\n");
		return 1;
	}
...
extern char xunhuSensorNameStr[KDIMGSENSOR_MAX_INVOKE_DRIVERS][32];
/*******************************************************************************
...
int imgsensor_set_driver(struct IMGSENSOR_SENSOR *psensor)
{
	...
	for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
		if (pSensorList[i].init) {
			pSensorList[i].init(&psensor->pfunc);
			if (psensor->pfunc) {
				/* get sensor name */
				psensor_inst->psensor_name = (char *)pSensorList[i].name;
#ifdef IMGSENSOR_LEGACY_COMPAT
				...
				if (!imgsensor_check_is_alive(psensor)) {
					PK_INFO("[imgsensor_set_driver] :[%d][%d][%s]\n",
								...
								i,
								psensor_inst->psensor_name);
					memset(xunhuSensorNameStr[psensor->inst.sensor_idx], 0, 32);
                    memcpy((char*)xunhuSensorNameStr[psensor->inst.sensor_idx],(char*)pSensorList[i].name,sizeof(pSensorList[i].name));
					ret = i;

					#if defined(CONFIG_PRIZE_HARDWARE_INFO)
					if (psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN)
					{
						strcpy(current_camera_info[0].chip,psensor_inst->psensor_name);    			
						sprintf(current_camera_info[0].id,"0x%04x",pSensorList[i].id);    			
						strcpy(current_camera_info[0].vendor,"unknow");
						 psensor->pfunc->SensorGetResolution(&psensorResolution);
						sprintf(current_camera_info[0].more,"%d*%d",psensorResolution.SensorFullWidth,psensorResolution.SensorFullHeight);
					}
					else
					{
						strcpy(current_camera_info[1].chip,psensor_inst->psensor_name);    			
						sprintf(current_camera_info[1].id,"0x%04x",pSensorList[i].id);    			
						strcpy(current_camera_info[1].vendor,"unknow");
						 psensor->pfunc->SensorGetResolution(&psensorResolution);
						sprintf(current_camera_info[1].more,"%d*%d",psensorResolution.SensorFullWidth,psensorResolution.SensorFullHeight);		
					}
...
/*modify by trf318, imgsensor set driver end*/
/*******************************************************************************
