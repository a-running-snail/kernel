...
#define PROC_NAME	"xunhu_cam"
char xunhuSensorNameStr[KDIMGSENSOR_MAX_INVOKE_DRIVERS][32] = {"no_sensor","no_sensor"};
...
//Description:yuv camera use as three  (XUNHU_LPS_AUXILIARYCAMERA_SUPPORT)
#if 1              //def XUNHU_LPS_AUXILIARYCAMERA_SUPPORT
char* auxiliarycamera_getmainname(void)
{
    return xunhuSensorNameStr[0];
}
EXPORT_SYMBOL(auxiliarycamera_getmainname);
#endif
//&&}}
...
{
	seq_printf(m, "%s %s\n", xunhuSensorNameStr[0],xunhuSensorNameStr[1]);
	return 0;
}
static int proc_xunhu_cam_open(struct inode *inode, struct file *file)
{
	return single_open(file, cam_debug_read, NULL);
};
static struct file_operations xunhu_cam_fops = {
	.owner = THIS_MODULE,
	.open  = proc_xunhu_cam_open,
	.read  = seq_read,
...
enum IMGSENSOR_RETURN imgsensor_proc_init(void)
{
	...
	proc_create(PROC_NAME, 0777, NULL,&xunhu_cam_fops);
