	...
	char *pcurr_idx)
{
	...
	//for_idx:  -1: ignore, 0:main , 1: pre, ....
	if(ppwr_seq->for_idx != -1)
	{
		if(ppwr_seq->for_idx != sensor_idx)
		{
			PK_DBG("img_name=%s, conf_ismain=%d, sensor_idx=%d", pcurr_idx, ppwr_seq->for_idx, sensor_idx);
	...
	while (ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
		...
		if (pwr_status == IMGSENSOR_HW_POWER_STATUS_ON &&
		   ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
			...
			if(((ppwr_info->pin==1)||(ppwr_info->pin==2))&&(pdev->set != NULL))
			{
				pdev->set(pdev->pinstance,
							1-sensor_idx,
							ppwr_info->pin,
							(Vol_High+Vol_Low-(ppwr_info->pin_state_on)));
			mdelay(ppwr_info->pin_on_delay);
			}
		}
