...
#endif
/* zhengjiang.zhu@Koobee..Camera.Driver  2018/04/28 add for flashlight node :koobee_flash*/
#include <linux/proc_fs.h>
/* zhengjiang.zhu@Koobee..Camera.Driver  2018/04/28 end for flashlight node :koobee_flash*/
...
static int fl_set_timeout(int type_index, int ct_index, int part_index, int level)
{
	...
	fl_pr_err("fl_dev_arg.channel =%d fl_dev_arg.arg=%d",fl_dev_arg.channel,fl_dev_arg.arg );
 ...
 static ssize_t flashlight_proc_write( struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char regBuf[64] = {'\0'};
	int flashlight_state = 0;
	int ret = 0;

	struct flashlight_operations *ppF;
	//struct flashlight_operations *ppF2;
	...
	fl_pr_info("flashlight_state =%d\n",flashlight_state);
	if(1 == flashlight_state)
	{
	#if 1
		// channel 1 setting
		...
		ppF = fl_ops[0][0][0];
		if (ppF == NULL){
		    fl_pr_err("ppF is NULL \n");
		    return -EFAULT;
		}

		if (ppF) {
			ppF->flashlight_set_driver();
			mutex_lock(&fl_mutex);
			fl_status[0][0][0] = 1;
			mutex_unlock(&fl_mutex);
		} else {
			fl_pr_info("Failed flashlight_set_driver channel 1\n");
			return -EFAULT;
		}

                //FLASH_IOC_SET_DUTY & timeout
		mutex_lock(&fl_mutex);
		ret = fl_set_level(0, 0, 0, 0);
		if (ret){
			mutex_unlock(&fl_mutex);
			return -EFAULT;
		}
		ret = fl_set_timeout(0,0,0,0);
		mutex_unlock(&fl_mutex);

		//FLASH_IOC_SET_ONOFF
		mutex_lock(&fl_mutex);
		ret = fl_enable(0, 0, 0, 1);
		mutex_unlock(&fl_mutex);
	#endif
		#if 0
		// channel 2 setting
		...
		if (ppF2 == NULL)
		{
		        fl_pr_err("ppF2 is NULL \n");
		        return 0;
		}
		//FLASHLIGHTIOC_X_SET_DRIVER
		mutex_unlock(&fl_mutex);
		if (ppF2) {
			ppF2->flashlight_set_driver();
			mutex_lock(&fl_mutex);
			fl_status[0][1][0] = 1;
			mutex_unlock(&fl_mutex);
		} else {
			fl_pr_info("Failed flashlight_set_driver channel 2\n");
			return -EFAULT;
		}

		//FLASH_IOC_SET_DUTY & timeout
		mutex_lock(&fl_mutex);
		ret = fl_set_level(0, 1, 0, 0);
		if (ret)
			return -EFAULT;
		ret = fl_set_timeout(0,1,0,0);
		mutex_unlock(&fl_mutex);

		//FLASH_IOC_SET_ONOFF
		mutex_lock(&fl_mutex);
		ret = fl_enable(0, 1, 0, 1);
		mutex_unlock(&fl_mutex);
		#endif 
	...
	else if(0 == flashlight_state)
	{
		...
		if (ppF) {
			ret = fl_enable(0, 0, 0, 0);
			mutex_lock(&fl_mutex);
			fl_status[0][0][0] = 0;
			mutex_unlock(&fl_mutex);
		} else {
			fl_pr_info("Failed set  flashlight channel 1 fail\n");
			return -EFAULT;
		}
		...
		// 
		//if (ppF2) {
		//	ret = fl_enable(0, 1, 0, 0);
		//	mutex_lock(&fl_mutex);
		//	fl_status[0][1][0] = 0;
		//	mutex_unlock(&fl_mutex);
		//} else {
		//	fl_pr_info("Failed set  flashlight channel 2 fail\n");
		//	return -EFAULT;
		//}
 ...
 static ssize_t flashlight_proc_read(struct file *filp, char __user *buff,
                        	size_t len, loff_t *data)
{
 ...
 static const struct file_operations flashlight_proc_fops = {
	 .write = flashlight_proc_write, 
	 .read = flashlight_proc_read,
};
 static int flash_proc_init(void)
{
	int ret=0;
	struct proc_dir_entry *proc_entry = proc_create_data( "koobee_flash", 0666, NULL,&flashlight_proc_fops, NULL);
	if (proc_entry == NULL)
	{
		ret = -ENOMEM;
	  	fl_pr_err("Error! Couldn't create koobee_flash proc entry\n");
	}
	return ret;
}
/* zhengjiang.zhu@Koobee..Camera.Driver  2018/04/28  end for flashlight node :koobee_flash*/
