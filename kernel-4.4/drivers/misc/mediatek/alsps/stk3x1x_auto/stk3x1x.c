...
static void stk3x1x_enint_thread_run(void);
...
{
	struct stk3x1x_priv *obj = (struct stk3x1x_priv *)container_of(work, struct stk3x1x_priv, stk_ps_tune0_work);		
#ifdef STK_GES		
	if(obj->ges_enabled)
...
{
	struct stk3x1x_priv *obj = g_stk3x1x_ptr;
	//struct stk3x1x_priv *obj = (struct stk3x1x_priv *)container_of(work, struct stk3x1x_priv, eint_work);
	int err;
...
err_i2c_rw:	
	...
	{
        retrycount = 3;
		while(retrycount && (err = stk3x1x_clear_intr(obj->client, flag_reg, disable_flag)))
		{
			APS_ERR("stk3x1x stk3x1x_clear_intr: %d retrycount=%d\n", err,retrycount);
			retrycount--;
		}
		if (retrycount == 0)
		{
		...
		//stk3x1x_clear_intr(obj->client, flag_reg, disable_flag);
	}
	msleep(30);
#ifdef MT6516
	MT6516_EINTIRQUnmask(CUST_EINT_ALS_NUM);      
...
static void stk3x1x_enint_thread_run(void)
{
    ...
    {
        APS_LOG("AndyLog stk3x1x_enint_thread_run error\n");
...
static ssize_t meng_show_validationtools_value(struct device_driver *ddri, char *buf)
{
	...
	if(stk3x1x_enable_ps(client, 1, 0)){
		APS_ERR("%s enable \n",__FUNCTION__);
        return 0;
	}else{
...
static DRIVER_ATTR(ps,      0755, stk3x1x_show_ps,    NULL);
...
#ifdef CUSTOM_KERNEL_SENSORHUB
	if (atomic_read(&stk3x1x_obj->init_done)) {
...
static int ps_enable_nodata(int en)
{
	...
	if(en)
		res = stk3x1x_enable_ps(stk3x1x_obj->client, 1, 1);
	else
		res = stk3x1x_enable_ps(stk3x1x_obj->client, 0, 1);
#endif /* #ifdef CUSTOM_KERNEL_SENSORHUB */
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&obj->psctrl_val, 0x71);
	...
	/*------------------------stk3x1x attribute file for debug--------------------------------------*/
...
static int stk3x1x_i2c_remove(struct i2c_client *client)
{
   ...
   if(err)
	{
		misc_deregister(&stk3x1x_device);
		//printk("stk3x1x_delete_attr fail: %d\n", err);
	} 
...
static int  stk3x1x_local_uninit(void)
{
	//struct stk_alsps_hw *hw = stk3x1x_get_cust_alsps_hw();
	//APS_FUN();    
//	stk3x1x_power(hw, 0);    
	APS_LOG("%s: delete driver\n", __func__); 
	i2c_del_driver(&stk3x1x_i2c_driver);

	//APS_LOG("%s: unregister device, client=%d\n", __func__,(int)stk3x1x_i2c_client); 
	//i2c_unregister_device(stk3x1x_i2c_client);
	//stk3x1x_i2c_client = NULL;
	return 0;
}
/*----------------------------------------------------------------------------*/
...
//	stk3x1x_get_addr(hw, &addr);
    ...
    {
		i2c_del_driver(&stk3x1x_i2c_driver);/* if register fail remove it    add by TRF118 */
        return -1;
    }
	return 0;
}
...
static int __init stk3x1x_init(void)
{
...
#else
	const char *name = "mediatek,stk3x1x_auto";
#endif
	...
	get_teksunhw_alsps_content(stk3x1x_init_info.name,&mTekhwAlspsCntx);
#endif
