...
static void stk3x3x_enint_thread_run(void);
...
/*----------------------------------------------------------------------------*/
	...
	#define STK_HT_N_CT	((mTekhwAlspsCntx.stk_ht_n_ct > 0) ? mTekhwAlspsCntx.stk_lt_n_ct : 40)
#else
...
//static struct i2c_board_info __initdata i2c_stk3x3x={ I2C_BOARD_INFO("stk3x3x", (0x90>>1))};
...
static DEFINE_MUTEX(stk3x3x_mutex);
...
enum {
	CMC_BIT_ALS_STK3X3X	= 1,
	CMC_BIT_PS_STK3X3X	   = 2,
} CMC_STK3X3X_BIT;
/*----------------------------------------------------------------------------*/
typedef enum {
...
#endif	
#ifdef STK_ALS_FIR
	struct data_stk3x3x_filter      fir;
	atomic_t                firlength;		
#endif
	uint16_t ir_code;
...
static int32_t stk3x3x_get_ir_value(struct stk3x3x_priv *obj);
#ifdef STK_TUNE0
...
#ifdef STK_GES		
static uint32_t stk3x3x_get_ges_value(struct stk3x3x_priv *obj, unsigned int *ges0, unsigned int *ges1, unsigned int *ges2);
static int32_t stk3x3x_enable_ges(struct i2c_client *client, int enable, int mode);
#endif
/*----------------------------------------------------------------------------*/
int stk3x3x_get_addr(struct alsps_hw *hw, struct stk3x3x_i2c_addr *addr)
{
	if(!hw || !addr)
	{
		return -EFAULT;
...
#ifdef STK_GES		
/*----------------------------------------------------------------------------*/
int stk3x3x_read_gsctrl(struct i2c_client *client, u8 *data)
{
	//struct stk3x3x_priv *obj = i2c_get_clientdata(client);    
	int ret = 0;
	u8 buf;
	...
	ret = stk3x3x_master_recv(client, STK_GSCTRL_REG, &buf, 0x01);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
/*----------------------------------------------------------------------------*/
#ifdef STK_GES		
int stk3x3x_read_flag2(struct i2c_client *client, u8 *data)
{
	//struct stk3x3x_priv *obj = i2c_get_clientdata(client);    
	int ret = 0;
	u8 buf;
	...
	ret = stk3x3x_master_recv(client, STK_FLAG2_REG, &buf, 0x01);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
int stk3x3x_read_id(struct i2c_client *client)
{
	...
	if(buf[0] == 0)
	{
		APS_ERR( "PID=0x0, please make sure the chip is stk3x3x!\n");
		return -2;			
	}		
...
/*----------------------------------------------------------------------------*/
#ifdef STK_GES		
int stk3x3x_write_gsctrl(struct i2c_client *client, u8 data)
{
	//struct stk3x3x_priv *obj = i2c_get_clientdata(client);        
	int ret = 0;

    ret = stk3x3x_master_send(client, STK_GSCTRL_REG, &data, 1);
	if (ret < 0)
	{
...
/*----------------------------------------------------------------------------*/
#ifdef STK_GES		
int stk3x3x_write_flag2(struct i2c_client *client, u8 data)
{
	//struct stk3x3x_priv *obj = i2c_get_clientdata(client);        
	int ret = 0;

    ret = stk3x3x_master_send(client, STK_FLAG2_REG, &data, 1);
	if (ret < 0)
	{
		APS_ERR("write ps = %d\n", ret);
		return -EFAULT;
...
/*----------------------------------------------------------------------------*/
#if 0
int stk3x3x_write_foffset(struct i2c_client *client, u16 ofset)
{
	struct stk3x3x_priv *obj = i2c_get_clientdata(client);        
	u8 buf[2];
	int ret = 0;
    ...
    ret = stk3x3x_master_send(client, obj->addr.data1_offset, &buf[0], 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
    ...
    ret = stk3x3x_master_send(client, obj->addr.data2_offset, &(buf[1]), 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
...
int stk3x3x_write_aoffset(struct i2c_client *client,  u16 ofset)
{
	struct stk3x3x_priv *obj = i2c_get_clientdata(client);        
	u8 buf[2];
	int ret = 0;
	u8 s_buf = 0, re_en;
    ret = stk3x3x_master_recv(client, obj->addr.state, &s_buf, 1);
	if (ret < 0)
	{
	...
	if(re_en)
	{
		...
		ret = stk3x3x_master_send(client, obj->addr.state, &s_buf, 1);
		if (ret < 0)
		{
			APS_ERR("write state = %d\n", ret);
			return -EFAULT;
    ...
    buf[0] = (u8) ((0xFF00 & ofset) >> 8);
    buf[1] = (u8) (0x00FF & ofset);	
    ret = stk3x3x_master_send(client, 0x0E, &buf[0], 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
    ...
    ret = stk3x3x_master_send(client, 0x0F, &(buf[1]), 1);
	if (ret < 0)
	{
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
...
static int stk3x3x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	if(obj->ges_enabled)
	{
		if(enable)
		{
			...
			stk3x3x_enable_ges(obj->client, 0, obj->ges_enabled);
...
//						APS_ERR("call hwmsen_get_interrupt_data fail = %d\n", err);
	...
	else
	{
		if(obj->re_enable_ges2)
		{
			...
			stk3x3x_enable_ges(obj->client, 1, cur);
		}
	}
#endif	
...
static int32_t stk3x3x_enable_ges(struct i2c_client *client, int enable, int mode)
{
	...
	if(enable)
	{
		...
		if((err = stk3x3x_write_wait(client, 0)))
		{
		...
		if(mode == 2)
		{
			err = stk3x3x_read_gsctrl(client, &reg);
			if(err < 0)
				return err;		
			reg &= 0xF3;
			if(obj->hw->polling_mode_ps == 1)
				...
				reg |= 0x0C;
			err = stk3x3x_write_gsctrl(client, reg);
			if(err < 0)
				return err;			
			if(obj->hw->polling_mode_ps == 1)
			{
...
//#if defined(MT6582)
		...
		err = stk3x3x_write_state(client, 0);
		if(err < 0)
			return err;	
...
{    
	...
	ret = stk3x3x_master_recv(stk3x3x_obj->client, 8, &buf[7], 2);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
#ifdef STK_GES		
static int32_t stk_ges_poll_func(struct stk3x3x_priv *obj)
{
	...
	{
		ret = stk3x3x_read_flag2(obj->client, &org_flag2_reg);
		if(ret < 0)
			return ret;	
		...
		if(disable_flag2)
		{
			...
			ret = stk3x3x_write_flag2(obj->client, w_flag2_reg);	
			if(ret < 0)
				return ret;
		}			
	}		
...
{
	struct stk3x3x_priv *obj = (struct stk3x3x_priv *)container_of(work, struct stk3x3x_priv, stk_ps_tune0_work);		
#ifdef STK_GES		
	if(obj->ges_enabled)
...
{
	struct stk3x3x_priv *obj = g_stk3x3x_ptr;
...
{
	struct stk3x3x_priv *obj = g_stk3x3x_ptr;
	//struct stk3x3x_priv *obj = (struct stk3x3x_priv *)container_of(work, struct stk3x3x_priv, eint_work);
	int err;
...
err_i2c_rw:	
	...
	{
        retrycount = 3;
		while(retrycount && (err = stk3x3x_clear_intr(obj->client, flag_reg, disable_flag)))
		{
			APS_ERR("stk3x3x stk3x3x_clear_intr: %d retrycount=%d\n", err,retrycount);
			retrycount--;
		}
		if (retrycount == 0)
		{
		...
		//stk3x3x_clear_intr(obj->client, flag_reg, disable_flag);
	}
	msleep(30);
#ifdef MT6516
	MT6516_EINTIRQUnmask(CUST_EINT_ALS_NUM);      
...
static void stk3x3x_enint_thread_run(void)
{
    ...
    {
        APS_LOG("AndyLog stk3x3x_enint_thread_run error\n");
...
/* eint request */
	if (stk3x3x_obj->irq_node) {
		...
		if (request_irq(stk3x3x_obj->irq, stk3x3x_eint_handler, IRQF_TRIGGER_NONE, "als-eint", NULL)) {
...
*/	
	...
	//INTEL PEERS
	buf = 0x3F;
	err = stk3x3x_master_send(client, 0x4F, &buf, 1);
	if (err < 0)
	{
	...
	//PS INT mode
	buf = 0x01;
	err = stk3x3x_master_send(client, 0xFA, &buf, 1);
	if (err < 0)
	{
		APS_ERR("write i2c failed = %d\n", err);
	} 	
	...
	//BGIR
	buf = 0x30;
	err = stk3x3x_master_send(client, 0xA0, &buf, 1);
	if (err < 0)
	{
		APS_ERR("write i2c failed = %d\n", err);
	} 
	...
	buf = 0x64;
	err = stk3x3x_master_send(client, 0xAA, &buf, 1);
	if (err < 0)
	{
		APS_ERR("write i2c failed = %d\n", err);
	} 			
	...
	//BGIR
	buf = 0x15;
	err = stk3x3x_master_send(client, 0xDB, &buf, 1);
	if (err < 0)
	{
		APS_ERR("write i2c failed = %d\n", err);
	} 
...
static ssize_t stk3x3x_show_config(struct device_driver *ddri, char *buf)
{
	...
	res = scnprintf(buf, PAGE_SIZE, "(%d %d %d %d %d %d)\n", 
		atomic_read(&stk3x3x_obj->i2c_retry), atomic_read(&stk3x3x_obj->als_debounce), 
		atomic_read(&stk3x3x_obj->ps_mask), atomic_read(&stk3x3x_obj->ps_high_thd_val),atomic_read(&stk3x3x_obj->ps_low_thd_val), atomic_read(&stk3x3x_obj->ps_debounce));     
	return res;    
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x3x_show_ir(struct device_driver *ddri, char *buf)
{
    int32_t reading;
    ...
    reading = stk3x3x_get_ir_value(stk3x3x_obj);
	if(reading < 0)
		return scnprintf(buf, PAGE_SIZE, "ERROR: %d\n", reading);

	stk3x3x_obj->ir_code = reading;
...
static ssize_t stk3x3x_store_send(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	stk3x3x_master_send(stk3x3x_obj->client, (u16)addr, &dat, sizeof(dat)));
...
static ssize_t stk3x3x_show_status(struct device_driver *ddri, char *buf)
{
	...
	if(stk3x3x_obj->hw)
	{
		len += scnprintf(buf+len, PAGE_SIZE-len, "CUST: %d, (%d %d) (%02X) (%02X %02X %02X) (%02X %02X %02X %02X)\n", 
			stk3x3x_obj->hw->i2c_num, stk3x3x_obj->hw->power_id, stk3x3x_obj->hw->power_vol, stk3x3x_obj->addr.flag, 
			stk3x3x_obj->addr.alsctrl, stk3x3x_obj->addr.data1_als, stk3x3x_obj->addr.data2_als, stk3x3x_obj->addr.psctrl, 
			stk3x3x_obj->addr.data1_ps, stk3x3x_obj->addr.data2_ps, stk3x3x_obj->addr.thdh1_ps);
	}
	else
	{
	...
	len += scnprintf(buf+len, PAGE_SIZE-len, "REGS: %02X %02X %02X %02X %02X %02X %02X %02X %02lX %02lX\n", 
				atomic_read(&stk3x3x_obj->state_val), atomic_read(&stk3x3x_obj->psctrl_val), atomic_read(&stk3x3x_obj->alsctrl_val), 
				stk3x3x_obj->ledctrl_val, stk3x3x_obj->int_val, stk3x3x_obj->wait_val, 
				atomic_read(&stk3x3x_obj->ps_high_thd_val), atomic_read(&stk3x3x_obj->ps_low_thd_val),stk3x3x_obj->enable, stk3x3x_obj->pending_intr);
#ifdef MT6516
	...
	len += scnprintf(buf+len, PAGE_SIZE-len, "MISC: %d %d\n", atomic_read(&stk3x3x_obj->als_suspend), atomic_read(&stk3x3x_obj->ps_suspend));	
...
#ifdef STK_ALS_FIR
/*----------------------------------------------------------------------------*/
static ssize_t stk3x3x_show_firlen(struct device_driver *ddri, char *buf)
{
	int len = atomic_read(&stk3x3x_obj->firlength);
	...
	APS_LOG("%s: len = %2d, idx = %2d\n", __func__, len, stk3x3x_obj->fir.idx);			
	APS_LOG("%s: sum = %5d, ave = %5d\n", __func__, stk3x3x_obj->fir.sum, stk3x3x_obj->fir.sum/len);
	
	return scnprintf(buf, PAGE_SIZE, "%d\n", len);		
}

/*----------------------------------------------------------------------------*/
static ssize_t stk3x3x_store_firlen(struct device_driver *ddri, const char *buf, size_t count)
{
	int value;
	...
	{
		atomic_set(&stk3x3x_obj->firlength, 1);
		memset(&stk3x3x_obj->fir, 0x00, sizeof(stk3x3x_obj->fir));
	}
	else
	{ 
		atomic_set(&stk3x3x_obj->firlength, value);
		memset(&stk3x3x_obj->fir, 0x00, sizeof(stk3x3x_obj->fir));
	}
...
static ssize_t meng_show_validationtools_value(struct device_driver *ddri, char *buf)
{
	...
	if(stk3x3x_enable_ps(client, 1, 0)){
		APS_ERR("%s enable \n",__FUNCTION__);
        return 0;
	}else{
...
static DRIVER_ATTR(ps,      0755, stk3x3x_show_ps,    NULL);
static DRIVER_ATTR(ir,      S_IWUSR | S_IRUGO, stk3x3x_show_ir,    NULL);
...
#endif
#ifdef STK_ALS_FIR
static DRIVER_ATTR(firlen,    S_IWUSR | S_IRUGO, stk3x3x_show_firlen,  stk3x3x_store_firlen);
#endif
...
{
	struct stk3x3x_priv *obj =  dev_get_drvdata(dev);	
    int ret;
	unsigned int gest0 = 0, gest1 = 0, gest2 = 0;
	
	if(obj->ges_enabled)
		ret = stk3x3x_get_ges_value(obj, &gest0, &gest1, &gest2);
	else
		ret = 0;
...
static ssize_t stk_ges_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case 0:
		if(obj->ges_enabled == 1)
			stk3x3x_enable_ges(obj->client, 0, 1);
		else
			stk3x3x_enable_ges(obj->client, 0, 2);
		break;
	case 1:
		stk3x3x_enable_ges(obj->client, 1, 1);
		break;
	case 2:
		stk3x3x_enable_ges(obj->client, 1, 2);
		break;
	default:
		APS_ERR( "%s, invalid value %d\n", __func__, *buf);
		return -EINVAL;	
...
static ssize_t stk_recv_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	stk3x3x_master_recv(stk3x3x_obj->client, (u16)addr, (char*)&dat, sizeof(dat));
	//APS_LOG("recv(%02X) = %d, 0x%02X\n", addr, 
	//stk3x3x_master_recv(stk3x3x_obj->client, (u16)addr, (char*)&dat, sizeof(dat)), dat);
	atomic_set(&stk3x3x_obj->recv_reg, dat);	
	return size;
...
static uint32_t stk3x3x_get_ges_value(struct stk3x3x_priv *obj, unsigned int *ges0, unsigned int *ges1, unsigned int *ges2)
{
	...
	err = stk3x3x_master_recv(obj->client, obj->addr.data1_ps, buf, 0x02);
	if(err < 0)
	{
		APS_DBG("error: %d\n", err);
		return -EFAULT;
	...
	err = stk3x3x_master_recv(obj->client, 0x24, buf, 0x04);
	if(err < 0)
	{
		APS_DBG("error: %d\n", err);
		return -EFAULT;
...
static int32_t stk3x3x_set_irs_it_slp(struct stk3x3x_priv *obj, uint16_t *slp_time)
{
	uint8_t irs_alsctrl;
	int32_t ret;
...
static int32_t stk3x3x_get_ir_value(struct stk3x3x_priv *obj)
{
	...
	ret = stk3x3x_set_irs_it_slp(obj, &irs_slp_time);
	if(ret < 0)
		goto irs_err_i2c_rw;
	...
	ret = stk3x3x_clear_intr(obj->client, flag, STK_FLG_IR_RDY_MASK);	
    if (ret < 0)
	{
		APS_ERR( "%s: write i2c error\n", __func__);
		goto irs_err_i2c_rw;
	...
	ret = stk3x3x_master_recv(obj->client, STK_DATA1_IR_REG, buf, 2);
	if(ret < 0)	
	{
	...
	if(re_enable_ps)
		stk3x3x_enable_ps(obj->client, 1, 0);		
	return word_data;
...
static long stk3x3x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		case ALSPS_SET_PS_MODE:
			...
			if(enable)
			{
				err = stk3x3x_enable_ps(obj->client, 1, 1);
				if (err){
			...
			{
				err = stk3x3x_enable_ps(obj->client, 0, 1);
				if (err){
...
/*----------------------------------------------------------------------------*/
static void stk3x3x_early_suspend(struct early_suspend *h) 
{   /*early_suspend is only applied for ALS*/
	int err;
	struct stk3x3x_priv *obj = container_of(h, struct stk3x3x_priv, early_drv);   	
...
static int als_enable_nodata(int en)
{
...
#ifdef CUSTOM_KERNEL_SENSORHUB
	if (atomic_read(&stk3x3x_obj->init_done)) {
	...
	mutex_lock(&stk3x3x_mutex);
	if (en)
		set_bit(CMC_BIT_ALS_STK3X3X, &stk3x3x_obj->enable);
	else
		clear_bit(CMC_BIT_ALS_STK3X3X, &stk3x3x_obj->enable);
	mutex_unlock(&stk3x3x_mutex);
...
static int ps_enable_nodata(int en)
{
	...
	mutex_lock(&stk3x3x_mutex);
	if (en)
		set_bit(CMC_BIT_PS_STK3X3X, &stk3x3x_obj->enable);
	else
		clear_bit(CMC_BIT_PS_STK3X3X, &stk3x3x_obj->enable);
	mutex_unlock(&stk3x3x_mutex);
#else /* #ifdef CUSTOM_KERNEL_SENSORHUB */
	mutex_lock(&stk3x3x_mutex);
	if (en)
	...
	if(en)
		res = stk3x3x_enable_ps(stk3x3x_obj->client, 1, 1);
	else
		res = stk3x3x_enable_ps(stk3x3x_obj->client, 0, 1);
#endif /* #ifdef CUSTOM_KERNEL_SENSORHUB */
...
static int ps_get_data(int *value, int *status)
{
	...
	err = stk3x3x_read_ps(stk3x3x_obj->client, &stk3x3x_obj->ps);
	if (err)
		err = -1;
	else {
...
#if 0
static void stk3x3x_late_resume(struct early_suspend *h)
{   /*early_suspend is only applied for ALS*/
	int err;
	struct stk3x3x_priv *obj = container_of(h, struct stk3x3x_priv, early_drv);         	
	APS_FUN();
...
#endif
int stk3x3x_ps_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	struct hwm_sensor_data* sensor_data;
	struct stk3x3x_priv *obj = (struct stk3x3x_priv *)self;
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				{
					value = stk3x3x_get_ps_value(obj, obj->ps);
					if(value < 0)
					{
						err = -1;
					}
...
int stk3x3x_als_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{			
				...
				if(obj->ges_enabled)
				{
					sensor_data->values[0] = stk3x3x_get_als_value(obj, obj->als_last);				
					sensor_data->value_divide = 1;
					sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;				
					break;
				}
				...
				else
				{
					if(obj->als < 3)
					{
						...
						sensor_data->values[0] = stk3x3x_get_als_value(obj, 0);
					}
...
/*----------------------------------------------------------------------------*/
#ifdef STK_GES
static int stk3x3x_set_input_device(struct stk3x3x_priv *obj)
{
	int err;
...
static int stk3x3x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	stk3x3x_obj = obj;
	obj->hw = hw;
	stk3x3x_get_addr(obj->hw, &obj->addr);
...
#endif		
	err = stk3x3x_init_client(client);
	if (err)
	{
		goto exit_init_failed;
	}
	//alsps_compatible_flag=1;//add for  compatible by luolq
	err = misc_register(&stk3x3x_device);
	if (err){
		APS_ERR("stk3x3x_device register failed\n");
		goto exit_misc_device_register_failed;
	}
	err = stk3x3x_create_attr(&(stk3x3x_init_info.platform_diver_addr->driver));
	if (err){
		APS_ERR("create attribute err = %d\n", err);
		goto exit_create_attr_failed;
	}
	/*------------------------stk3x3x attribute file for debug--------------------------------------*/
	...
	obj_ps.sensor_operate = stk3x3x_ps_operate;
	...
	obj_als.sensor_operate = stk3x3x_als_operate;
...
#endif	
#ifdef STK_GES
	err = stk3x3x_set_input_device(obj);
	if(err < 0)
	...
	//prize-lixuefeng-20150512-start
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	strcpy(current_alsps_info.chip,"stk3x3x");
...
static int stk3x3x_i2c_remove(struct i2c_client *client)
{
	...
	input_free_device(obj->ges_input_dev);		
#endif	
	err = stk3x3x_delete_attr(&(stk3x3x_init_info.platform_diver_addr->driver));
   if(err)
	{
		misc_deregister(&stk3x3x_device);
		//printk("stk3x3x_delete_attr fail: %d\n", err);
	} 
...
static int  stk3x3x_local_uninit(void)
{
	//struct stk_alsps_hw *hw = stk3x3x_get_cust_alsps_hw();
	//APS_FUN();    
//	stk3x3x_power(hw, 0);    
	APS_LOG("%s: delete driver\n", __func__); 
	i2c_del_driver(&stk3x3x_i2c_driver);

	//APS_LOG("%s: unregister device, client=%d\n", __func__,(int)stk3x3x_i2c_client); 
	//i2c_unregister_device(stk3x3x_i2c_client);
	//stk3x3x_i2c_client = NULL;
	return 0;
}
/*----------------------------------------------------------------------------*/
...
//	stk3x3x_get_addr(hw, &addr);
    ...
    {
		i2c_del_driver(&stk3x3x_i2c_driver);/* if register fail remove it    add by TRF118 */
        return -1;
    }
	return 0;
}
...
static int __init stk3x3x_init(void)
{
...
#else
	const char *name = "mediatek,stk3x3x_auto";
#endif
	...
	get_teksunhw_alsps_content(stk3x3x_init_info.name,&mTekhwAlspsCntx);
#endif
	alsps_driver_add(&stk3x3x_init_info);
