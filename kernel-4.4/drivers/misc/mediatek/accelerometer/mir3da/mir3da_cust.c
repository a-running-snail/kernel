...
#define MIR3DA_STEP_C						   "MIR3DA_STEP_C"
...
//#define GSENSOR_PROXIMITY

#ifdef GSENSOR_PROXIMITY
...
//#define DEBUG_GSENSOR_PROXIMITY 
#ifdef DEBUG_GSENSOR_PROXIMITY
#define PROXIMITY_DEBUG(fmt,arg...) printk("<<gsensor_proximity>> "fmt"\n",##arg)
#else //DEBUG_GSENSOR_PROXIMITY
#define PROXIMITY_DEBUG(fmt,arg...) do { } while (0)
#endif //DEBUG_GSENSOR_PROXIMITY
static u8 gsensor_proximity_flag        = 0; //0-->off; 1--->on
static int gsensor_proximity_high_parm = 0x1c00;
static int gsensor_proximity_low_parm = 0x1b00;
static u8 gsensor_proximity_detect      = 15;//0-->close ; 1--> far away
static u8 gsensor_proximity_nearformfar         = 0; //0-->off; 1--->on

#endif //GSENSOR_PROXIMITY 1
...
#ifdef GSENSOR_PROXIMITY
static int gsensor_get_ps_value(void)
{
    PROXIMITY_DEBUG("gsensor_get_ps_value sensor_power=%d\n",sensor_power); 
    if (!sensor_power)
    {
        ...
        if(NULL == client)
        {
            PROXIMITY_DEBUG("i2c client is null!!\n");
            return 0;
        }
        PROXIMITY_DEBUG("show_sensordata_value\n");
        mir3da_readSensorData(client, strbuf);  
    }
    return gsensor_proximity_detect;
}

static int gsensor_enable_ps(int enable)
{
    //u8 state;
    //int ret = -1;
    if (enable){
        gsensor_proximity_flag = 1;
        PROXIMITY_DEBUG("[proxi_mir3da]ps function is on\n");   
    }else{
        gsensor_proximity_flag = 0;
        gsensor_proximity_nearformfar = 0;
        gsensor_proximity_detect = 15;
        PROXIMITY_DEBUG("[proxi_mir3da0]ps function is off\n");
    }

    PROXIMITY_DEBUG("[proxi_mir3da] gsensor_enable_ps \n");
    return 0;
}


static int mgsensor_ps_operate(void* self, uint32_t command, void* buff_in, int size_in,
        void* buff_out, int size_out, int* actualout)
{
  ...
  //  struct mxc400x_i2c_data *priv = (struct mxc400x_i2c_data*)self;
    ...
    PROXIMITY_DEBUG("[proxi_mir3da]command = 0x%02X\n", command);       
    switch (command)
    {
        case SENSOR_DELAY:
        ...
        case SENSOR_GET_DATA:
            ...
            else
            {
                ...
                {
                    sensor_data->values[0] = gsensor_get_ps_value();
                    PROXIMITY_DEBUG("wang sensor_data->values[0] y = %d \n", sensor_data->values[0]);
...
#ifdef GSENSOR_PROXIMITY
    if ((gsensor_proximity_nearformfar) && (acc[MIR3DA_AXIS_Z] < gsensor_proximity_high_parm))
    {
        gsensor_proximity_detect=0;//near  __
    }
    else if (acc[MIR3DA_AXIS_Z] > gsensor_proximity_low_parm)
    {
        gsensor_proximity_detect=15;//far |
        gsensor_proximity_nearformfar = 1;
    }

    if(gsensor_proximity_flag==0)
    {
        gsensor_proximity_detect=15;
    }

    PROXIMITY_DEBUG("gsensor_proximity_detect[far:1,near:0]=%d gsensor_proximity_flag[on:1,off:0]=%d\n",gsensor_proximity_detect,gsensor_proximity_flag);
#endif //GSENSOR_PROXIMITY 3    
    }
...
static ssize_t mir3da_step_show(struct device_driver *ddri, char *buf)
{
	...
	mir3da_get_step_data(handle , &step);
...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mir3da_step_count_init(client);
	mir3da_set_step_enable(client, 1);
    ...
    //sobj.self = obj;
    obj_ps.polling = 1;// 1-->no interrupt mode;       0--->interrupt mode
    obj_ps.sensor_operate = mgsensor_ps_operate;
    wake_lock_init(&ps_lock,WAKE_LOCK_SUSPEND,"mir3da_psensor_wakelock");
    if(hwmsen_attach(ID_PROXIMITY, &obj_ps)!=0)
    {
        PROXIMITY_DEBUG("proxi_gsensor attach fail \n");
        return -1;
    }
    else
    {
        PROXIMITY_DEBUG("proxi_gsensor attach ok \n");
    }       
#endif //GSENSOR_PROXIMITY 6

    mir3da_init_flag = 0;

#if defined(CONFIG_PRIZE_HARDWARE_INFO)
     strcpy(current_gsensor_info.chip,"da213");
     sprintf(current_gsensor_info.id,"0x%04x",client->addr);
     strcpy(current_gsensor_info.vendor,"mir3da");
...
#ifdef XUNHU_LPS_TEKHW_SUPPORT
	    int found = 0;
    tekhw_gsensor_data* ptr = tekhw_get_gsensor_data();
		const char *name = "mediatek,teksun_acc";
#else
	const char *name = "mediatek,mir3da"; //"mediatek,da226";	/* kcm modify by phf, 20160514 */
#endif	
//	if(accel_compatible_flag==1)
//		return 0;
    ...
    while(ptr != NULL)
    {
        if (strcmp(ptr->name.buffer, mir3da_init_info.name) == 0)
        {
            found = 1;
            break;
        }
        ptr = ptr->next_tekhw_gsensor_data;
    }
    if (found == 1)
    {
        hw->i2c_num = (ptr->i2c_num.size > 0) ? *(tekhw_u16*)ptr->i2c_num.buffer : hw->i2c_num;
        hw->direction = (ptr->direction.size > 0) ? *(tekhw_u16*)ptr->direction.buffer : hw->direction;
