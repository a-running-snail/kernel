...
int mir3da_step_count_init(MIR_HANDLE handle){
	...
	res |= mir3da_register_mask_write(handle, NSA_REG_STEP_CONFIG2,	0xff, 0xe2);
...
int mir3da_get_step_data(MIR_HANDLE handle, unsigned short* step)
{
	unsigned char step_temp[2]={0};
	int res=0;

	res = mir3da_register_read_continuously(handle, NSA_REG_STEPS_MSB, 2, step_temp);

	if(res)
		return -1;

	*step = ((step_temp[0]<<8) + step_temp[1])/2;
