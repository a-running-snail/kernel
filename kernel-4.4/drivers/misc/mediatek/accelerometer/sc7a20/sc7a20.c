...
#if 0
static int sc7a20_suspend(struct i2c_client *client, pm_message_t msg);
...
static const struct of_device_id accel_of_match[] = {
	{.compatible = "mediatek,gsensor_sc7a20"},//加_1（gsensor_1）代表会做兼容，和dts文件里面对应
	{},
};
#endif
...
#if 0
#define GSE_FUN                  pr_debug(KERN_INFO GSE_TAG"%s\n", __FUNCTION__)
...
static int SC7A20_CheckDeviceID(struct i2c_client *client)
{
	...
	if(res < 0)
	{
		GSE_ERR("SC7A20_CheckDeviceID read err\n ");
		goto exit_SC7A20_CheckDeviceID;
	}
	if(databuf[0]!=SC7A20_FIXED_DEVID)
	{
		GSE_ERR("SC7A20_CheckDeviceID %d failt!\n ", databuf[0]);
		return SC7A20_ERR_IDENTIFICATION;
	}
	else
	{
		GSE_LOG("SC7A20_CheckDeviceID %d pass!\n ", databuf[0]);
	}
...
static void sc7a20_early_suspend(struct early_suspend *h) 
{
	...
	atomic_set(&obj->suspend, 1); 
	/*
	if(err = hwmsen_write_byte(obj->client, SC7A20_REG_POWER_CTL, 0x00))
	{
		GSE_ERR("write power control fail!!\n");
		return;
	}  
...
static void sc7a20_late_resume(struct early_suspend *h)
{
	...
	SC7A20_power(obj->hw, 1);
	if(err = SC7A20_Init(obj->client, 0))
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
...
static int sc7a20_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err)
	{
        ...
        err = misc_register(&sc7a20_device);
	if(err)
	{
...
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
     strcpy(current_gsensor_info.chip,"sc7a20");
...
static int __init sc7a20_init(void)
{
...
#else
    const char *name = "mediatek,sc7a20";
#endif	
    ...
    while(ptr != NULL)
    {
        if (strcmp(ptr->name.buffer, sc7a20_init_info.name) == 0)
        {
            found = 1;
            break;
        }
