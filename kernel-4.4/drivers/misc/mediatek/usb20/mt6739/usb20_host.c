...
static struct hrtimer kick_wdt_kthread_timer;
static struct delayed_work kick_wdt_work;
static void mtk_kick_wdt_stop_timer(void)
{
	hrtimer_cancel(&kick_wdt_kthread_timer);
}
static void mtk_kick_wdt_start_timer(void)
{
	ktime_t ktime = ktime_set(POLLING_INTERVAL, 0);
	hrtimer_start(&kick_wdt_kthread_timer, ktime, HRTIMER_MODE_REL);
}

static void musb_kick_wdt_work(struct work_struct *data)
{
	charger_dev_kick_wdt(primary_charger);
	mtk_kick_wdt_start_timer();
}

static enum hrtimer_restart kick_wdt_hrtimer_func(struct hrtimer *timer)
{
	schedule_delayed_work(&kick_wdt_work, 0);
	return HRTIMER_NORESTART;
}

static void mtk_kick_wdt_init_timer(void)
{
	ktime_t ktime = ktime_set(POLLING_INTERVAL, 0);
	hrtimer_init(&kick_wdt_kthread_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	kick_wdt_kthread_timer.function = kick_wdt_hrtimer_func;
	hrtimer_start(&kick_wdt_kthread_timer, ktime, HRTIMER_MODE_REL);
}
#endif
...
#ifdef CONFIG_MTK_ETA6937_SUPPORT
	INIT_DELAYED_WORK(&kick_wdt_work, musb_kick_wdt_work);
#endif
